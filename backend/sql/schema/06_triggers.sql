...
COMPOUND TRIGGER
    ...
    BEGIN
        FOR i IN 1 .. "g_rows".COUNT LOOP
            ...
            ----------------------------------------------------------------
            -- 1) گرفتن ID تالی مربوط به قبض انبار
            ----------------------------------------------------------------
            BEGIN
                SELECT "h"."TALI_ID"
                INTO "v_tali_id"
            ...
            ----------------------------------------------------------------
            -- 2) محاسبه مجموع‌های ثبت شده در قبض انبار برای کالا در کل تالی
            ----------------------------------------------------------------
            SELECT
            ...
            ----------------------------------------------------------------
            -- 3) گرفتن مجموع مقادیر مجاز از تالی (حتی اگر چند ردیف باشد)
            ----------------------------------------------------------------
            SELECT
            ...
            ----------------------------------------------------------------
            -- 4) اگر در تالی هیچ رکوردی نبود → خطا
            ----------------------------------------------------------------
            IF "v_tali_number" = 0
            AND "v_tali_weight" = 0
