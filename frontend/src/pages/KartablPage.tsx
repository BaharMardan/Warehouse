...
// Same palette as the tally list badges, applied to whole rows here.
const STATUS = {
  open: { color: 'red', accent: '#e03131', label: 'تالی' },
  pending: { color: 'yellow', accent: '#f08c00', label: 'قبض انبار' },
  closed: { color: 'green', accent: '#2f9e44', label: 'صورتحساب' },
} as const
  ...
  return (
    <div dir="rtl">
      <style>{`
        .kt-row { background: var(--kt-bg); cursor: pointer; }
        .kt-row:hover { background: var(--kt-bg-hover); }
        .kt-row td:first-child { border-inline-start: 6px solid var(--kt-accent); }
      `}</style>
                  ...
                  return (
                    <Table.Tr
                      ...
                      style={{
                        ...
                        '--kt-bg': `color-mix(in srgb, ${status.accent} 15%, transparent)`,
                        '--kt-bg-hover': `color-mix(in srgb, ${status.accent} 26%, transparent)`,
                      } as CSSProperties}
                      onClick={() => navigate(tallyPath(row))}
                    >
                      <Table.Td>
